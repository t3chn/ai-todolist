...
Deployment target details are private and must not be committed. Use local/private deployment notes for host, user, and remote paths.

**Manual deploy (if make unavailable):**
```bash
docker run --rm --platform linux/amd64 -v "$(pwd)":/app -w /app rust:latest cargo build --release
ssh "$DEPLOY_TARGET" "systemctl stop ai-todolist"
scp target/release/ai-todolist "$DEPLOY_TARGET:$REMOTE_BIN"
ssh "$DEPLOY_TARGET" "systemctl start ai-todolist"
```
...
- `DATABASE_URL`: SQLite database URL for the bot runtime.
- `ADMIN_IDS`: comma-separated Telegram user IDs for `/admin`.
